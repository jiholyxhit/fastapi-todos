...
def test_user_sign_up(client, mocker):
    ...
    user_create = mocker.patch.object(
        User,
        "create",
        return_value = User(id=None, username="test", password="hashed"), #todos property will automatically be assigned with `relationship` method
    )
    mocker.patch.object(
        UserRepository,
        "save_user",
    ...
    assert response.json() == {"id": 1, "username": "test"} # returns UserSchema only with properties of id, username,
